...
func TestFactoryBuildsOpenAICompatibleClient(t *testing.T) {
	...
	var gotPath string
	factory, err := NewFactory(roundTripFunc(func(request *http.Request) (*http.Response, error) { gotMethod = request.Method; gotPath = request.URL.Path; return response(200, "ok"), nil }), mustTestClassifierRegistry(t))
...
func TestFactoryRejectsWrongAPIFamily(t *testing.T) {
	factory, err := NewFactory(roundTripFunc(func(*http.Request) (*http.Response, error) { return nil, nil }), mustTestClassifierRegistry(t))
	if err != nil { t.Fatalf("NewFactory: %v", err) }
	input := ports.ForwardingAdapterFactoryInput{Route: baseRoute(domain.EndpointChat), Reseller: baseReseller(), ResellerAPIKey: resellerSecret, MaxResponseBodyBytes: 64}
	input.Route.APIFamily = domain.APIFamilyGeminiNative
	_, err = factory.Build(input)
	if !errors.Is(err, ErrUnsupportedRoute) { t.Fatalf("error=%v", err) }
}
...
func TestFactoryRejectsProviderWithoutClassifier(t *testing.T) {
	classifiers, err := NewClassifierRegistry(ClassifierRegistration{ProviderType: domain.ProviderOpenAI, Classifier: StatusClassifier{}})
	if err != nil { t.Fatalf("NewClassifierRegistry: %v", err) }
	factory, err := NewFactory(roundTripFunc(func(*http.Request) (*http.Response, error) { return response(http.StatusOK, "ok"), nil }), classifiers)
	if err != nil { t.Fatalf("NewFactory: %v", err) }
	input := ports.ForwardingAdapterFactoryInput{Route: baseRoute(domain.EndpointChat), Reseller: baseReseller(), ResellerAPIKey: resellerSecret, MaxResponseBodyBytes: 64}
	input.Route.ProviderType = domain.ProviderHydra
	input.Reseller.ProviderType = domain.ProviderHydra
	_, err = factory.Build(input)
	if !errors.Is(err, ErrClassifierNotRegistered) { t.Fatalf("error=%v", err) }
}

func mustTestClassifierRegistry(t *testing.T) *ClassifierRegistry {
	t.Helper()
	registry, err := NewClassifierRegistry(ClassifierRegistration{ProviderType: domain.ProviderOpenAI, Classifier: StatusClassifier{}})
	if err != nil { t.Fatalf("NewClassifierRegistry: %v", err) }
	return registry
}
