...
func TestFactorySupportsGeminiChatAndEmbeddings(t *testing.T) {
	factory, err := NewFactory(roundTripFunc(func(*http.Request) (*http.Response, error) { t.Fatal("transport must not be called"); return nil, nil }))
	if err != nil { t.Fatal(err) }
	if !factory.SupportsForwardingEndpoint(domain.EndpointChat) || !factory.SupportsForwardingEndpoint(domain.EndpointEmbeddings) || factory.SupportsForwardingEndpoint(domain.EndpointImagesGeneration) { t.Fatalf("unexpected endpoint support") }
}
...
func TestFactoryBuildsGeminiForwardingClient(t *testing.T) {
	...
	client, err := factory.Build(ports.ForwardingAdapterFactoryInput{
		...
		ResellerAPIKey: "sk_provider_secret", MaxResponseBodyBytes: 1024,
	})
	...
	if response.StatusCode != http.StatusOK || upstreamPath != "/v1beta/models/gemini-client:generateContent" { t.Fatalf("response=%+v upstreamPath=%q", response, upstreamPath) }
}
...
func TestFactoryRejectsUnsupportedGeminiRoute(t *testing.T) {
	...
	_, err = factory.Build(ports.ForwardingAdapterFactoryInput{
		Route: geminiRoute(domain.EndpointImagesGeneration, "image-client", "image-client", domain.ModelRewritePolicyNone),
		Reseller: domain.Reseller{ID: "reseller-gemini", ProviderType: domain.ProviderGemini, BaseURL: "https://gemini.example"},
		ResellerAPIKey: "sk_provider_secret", MaxResponseBodyBytes: 1024,
	})
	if err == nil { t.Fatal("expected unsupported route") }
}
