...
func (f *Factory) Build(
	...
	if input.Route.APIFamily != domain.APIFamilyOllamaNative ||
		input.Route.ProviderType != domain.ProviderOllama ||
	...
	if err != nil {
		return nil, fmt.Errorf(
			"construct ollama native forwarding adapter: %w",
			err,
		)
	}
	...
	if err != nil {
		return nil, fmt.Errorf(
			"construct ollama native forwarding client: %w",
			err,
		)
	}
	return client, nil
