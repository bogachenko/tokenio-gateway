...
func TestFactorySupportsOnlyAnthropicChatEndpoint(t *testing.T) {
	...
	if !factory.SupportsForwardingEndpoint(domain.EndpointChat) {
		t.Fatal("chat endpoint unsupported")
	}
	for _, endpointKind := range []domain.EndpointKind{
		domain.EndpointEmbeddings,
		domain.EndpointImagesGeneration,
		domain.EndpointModels,
	} {
		if factory.SupportsForwardingEndpoint(endpointKind) {
			t.Fatalf("%s endpoint unexpectedly supported", endpointKind)
		}
	}
}
...
func TestFactoryBuildsAnthropicMessagesClient(t *testing.T) {
	...
	factory, err := NewFactory(roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		var readErr error
		seenBody, readErr = io.ReadAll(request.Body)
		if readErr != nil {
		...
		return &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(
				`{"id":"msg_1","usage":{"input_tokens":11,"output_tokens":7}}`,
			)),
		}, nil
	}))
	if err != nil {
	...
	response, err := client.Forward(context.Background(), ports.ForwardingClientRequest{
		Route: input.Route,
		Body:  body,
	})
	if err != nil {
	...
	if seenURL != "https://anthropic.example/root/v1/messages" {
		t.Fatalf("seen URL = %q", seenURL)
	}
	if seenHeader.Get("x-api-key") != resellerSecret {
		t.Fatalf("x-api-key = %q", seenHeader.Get("x-api-key"))
	}
	if seenHeader.Get("Content-Type") != "application/json" {
		t.Fatalf("Content-Type = %q", seenHeader.Get("Content-Type"))
	}
	if string(seenBody) != string(body) {
		t.Fatalf("body changed: %s", seenBody)
	}
	if string(response.Body) != `{"id":"msg_1","usage":{"input_tokens":11,"output_tokens":7}}` {
		t.Fatalf("response body changed: %s", response.Body)
	}
...
func TestFactoryRejectsCrossFamilyOrUnsupportedRoutes(t *testing.T) {
	...
	tests := []struct {
		name   string
		mutate func(*ports.ForwardingAdapterFactoryInput)
	}{
		{
			name: "openai compatible family",
			mutate: func(input *ports.ForwardingAdapterFactoryInput) {
				input.Route.APIFamily = domain.APIFamilyOpenAICompatible
			},
		},
		{
			name: "unsupported endpoint",
			mutate: func(input *ports.ForwardingAdapterFactoryInput) {
				input.Route.EndpointKind = domain.EndpointEmbeddings
			},
		},
		{
			name: "provider mismatch",
			mutate: func(input *ports.ForwardingAdapterFactoryInput) {
				input.Route.ProviderType = domain.ProviderType("other")
			},
		},
		{
			name: "reseller mismatch",
			mutate: func(input *ports.ForwardingAdapterFactoryInput) {
...
func TestFactoryRejectsInvalidConfig(t *testing.T) {
	if _, err := NewFactory(nil); !errors.Is(err, ErrInvalidAdapterConfig) {
		t.Fatalf("err = %v", err)
	}
}
