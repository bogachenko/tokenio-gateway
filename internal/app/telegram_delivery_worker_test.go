...
func TestNewWorkerGraphWiresTelegramDeliveryOnlyWhenConfigured(
	t *testing.T,
) {
	t.Run("disabled", func(t *testing.T) {
		...
		if graph.TelegramDeliveryEnabled ||
			...
			t.Fatalf("disabled Telegram delivery worker = %+v", graph)
		}
	})
	...
	t.Run("enabled", func(t *testing.T) {
		...
		if !graph.TelegramDeliveryEnabled ||
			graph.TelegramDelivery == nil {
			t.Fatalf("enabled Telegram delivery worker = %+v", graph)
		}
	})
}
