...
type testRecoverer struct {
	calls      []int
	results    []telegramalert.StaleAttemptRecoveryResult
	errors     []error
...
func (r *testRecoverer) Recover(
	...
	index := len(r.calls) - 1
	var result telegramalert.StaleAttemptRecoveryResult
...
func (o *testObserver) ObserveTelegramStaleAttemptRecoveryCycle(
	cycle Cycle,
) {
	o.mu.Lock()
...
func TestWorkerRunsInitialCycleBeforeTicker(t *testing.T) {
	...
	if !reflect.DeepEqual(recoverer.calls, []int{17}) {
		t.Fatalf("recover calls = %#v", recoverer.calls)
	}
	...
	if factoryCalls != 0 {
		t.Fatal("ticker created after canceled initial cycle")
	}
}

func TestWorkerObservesCycleErrorAndContinues(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	...
	if !errors.Is(cycles[0].Err, cycleErr) {
		t.Fatalf("first cycle error = %v", cycles[0].Err)
	}
	if cycles[1].Err != nil {
		t.Fatalf("second cycle error = %v", cycles[1].Err)
	}
	if !ticker.stopped {
...
func TestWorkerStopsBeforeInitialCycleWhenContextCanceled(
	t *testing.T,
) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	...
	if len(recoverer.calls) != 0 ||
		len(observer.snapshot()) != 0 {
		t.Fatal("canceled worker executed a cycle")
	}
}
...
var _ Recoverer = (*testRecoverer)(nil)
