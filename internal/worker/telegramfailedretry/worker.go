...
var ErrInvalidWorkerConfig = errors.New(
	"invalid Telegram failed-retry worker config",
)

type Recoverer interface {
	RecoverFailed(context.Context, int) (telegramalert.RecoveryResult, error)
}
...
type Observer interface {
	ObserveTelegramFailedRetryCycle(Cycle)
}
...
func (w *Worker) runCycle(ctx context.Context) {
	result, err := w.recoverer.RecoverFailed(ctx, w.batchSize)
	w.observer.ObserveTelegramFailedRetryCycle(Cycle{
		Result: result,
		Err:    err,
	})
}
