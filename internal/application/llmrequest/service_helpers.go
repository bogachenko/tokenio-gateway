...
func cloneAutoChargeResult(value AutoChargeResult) AutoChargeResult {
	result := value
	result.ProcessedBatchIDs = append([]string(nil), value.ProcessedBatchIDs...)
	if value.BillingBalanceCents != nil {
		copied := *value.BillingBalanceCents
		result.BillingBalanceCents = &copied
	}
	switch result.Status {
	case AutoChargeStatusDeferred, AutoChargeStatusProcessed, AutoChargeStatusFailed:
	default:
		result = AutoChargeResult{Status: AutoChargeStatusFailed}
	}
...
func validatePrincipal(value Principal) error {
	if strings.TrimSpace(value.UserID) == "" ||
		...
		return fmt.Errorf("%w: invalid authentication principal", ErrStageContractViolation)
	}
	return nil
}

func validateRoutePlan(input Input, parsed ParsedRequest, plan RoutePlan) error {
	route := plan.Route
	...
	if strings.TrimSpace(route.ID) == "" ||
		...
		return fmt.Errorf("%w: invalid route plan", ErrStageContractViolation)
	}
	return nil
}

func validateBillingAdmission(prepared PreparedRequest, admission BillingAdmissionResult) error {
	if !admission.Allowed ||
		admission.Currency != prepared.Plan.Currency ||
		admission.RequiredReserveCents != prepared.Plan.EstimatedClientAmountCents ||
		admission.RemoteBalanceCents < 0 ||
		admission.PendingAmountCents < 0 ||
		admission.RemoteBalanceCents < admission.PendingAmountCents ||
		admission.EffectiveBalanceCents != admission.RemoteBalanceCents-admission.PendingAmountCents ||
		admission.EffectiveBalanceCents < admission.RequiredReserveCents {
		return fmt.Errorf("%w: invalid billing admission", ErrStageContractViolation)
	}
	return nil
}

func reservationInput(prepared PreparedRequest) ReservationInput {
	return ReservationInput{
		LocalRequestID: prepared.LocalRequestID,
		IdempotencyKey: cloneStringPointer(prepared.IdempotencyKey),
...
func validateReservation(prepared PreparedRequest, result ReservationResult) error {
	usage := result.Usage
	expectedIdempotencyKey := ""
	if prepared.IdempotencyKey != nil {
		expectedIdempotencyKey = *prepared.IdempotencyKey
	}
	if result.Disposition != ReservationDispositionCreated && result.Disposition != ReservationDispositionAlreadyReserved ||
		...
		return fmt.Errorf("%w: invalid atomic reservation", ErrStageContractViolation)
	}
	switch result.Disposition {
	case ReservationDispositionCreated:
		if result.Reseller == nil ||
			result.Reseller.ID != prepared.Plan.Reseller.ID ||
			result.Reseller.ProviderType != prepared.Plan.Reseller.ProviderType ||
			result.Reseller.ReservedCents < prepared.Plan.EstimatedUpstreamCostCents {
			return fmt.Errorf("%w: invalid created reseller reserve", ErrStageContractViolation)
		}
	case ReservationDispositionAlreadyReserved:
		if result.Reseller != nil {
			return fmt.Errorf("%w: replay returned mutable reseller snapshot", ErrStageContractViolation)
		}
	}
	return nil
}
...
func validLocalRequestID(value string) bool {
	if !strings.HasPrefix(value, localRequestIDPrefix) || len(value) == len(localRequestIDPrefix) {
		return false
	}
...
func validUpstreamPath(value string) bool {
...
func clonePreparedRequest(value PreparedRequest) PreparedRequest {
	result := value
	result.IdempotencyKey = cloneStringPointer(value.IdempotencyKey)
	result.Payload = cloneBytes(value.Payload)
	result.Plan.Fallbacks = cloneRouteFallbackPlans(value.Plan.Fallbacks)
	return result
}

func cloneRouteFallbackPlans(values []RouteFallbackPlan) []RouteFallbackPlan {
	if values == nil {
		return nil
	}
...
func cloneReservationResult(value ReservationResult) ReservationResult {
	result := value
	result.Usage.ReservedAt = cloneTimePointer(value.Usage.ReservedAt)
	result.Usage.ReleasedAt = cloneTimePointer(value.Usage.ReleasedAt)
	result.Usage.BillableAt = cloneTimePointer(value.Usage.BillableAt)
	result.Usage.ChargedAt = cloneTimePointer(value.Usage.ChargedAt)
	result.Usage.FailedAt = cloneTimePointer(value.Usage.FailedAt)
	if value.Reseller != nil {
		reseller := *value.Reseller
		reseller.DisabledAt = cloneTimePointer(value.Reseller.DisabledAt)
