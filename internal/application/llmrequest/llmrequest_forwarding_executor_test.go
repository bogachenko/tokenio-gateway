...
type executorFactoryFunc func(
	ports.ForwardingAdapterFactoryInput,
) (ports.ForwardingClient, error)

func (function executorFactoryFunc) Build(
	input ports.ForwardingAdapterFactoryInput,
) (ports.ForwardingClient, error) {
	return function(input)
}

type executorClientFunc func(
	context.Context,
	ports.ForwardingClientRequest,
) (ports.ForwardResponse, error)

func (function executorClientFunc) Forward(
	ctx context.Context,
	input ports.ForwardingClientRequest,
) (ports.ForwardResponse, error) {
...
func TestLLMRequestForwardingExecutorUsesSemanticBoundary(t *testing.T) {
	...
	if gotSecretName != input.Prepared.Plan.Reseller.APIKeyEnv {
		t.Fatalf("secret name=%q", gotSecretName)
	}
	if gotFactory.Route.ID != input.Prepared.Plan.Route.ID ||
		gotFactory.Reseller.ID != input.Prepared.Plan.Reseller.ID ||
		gotFactory.ResellerAPIKey != "resolved-secret" ||
		gotFactory.MaxResponseBodyBytes != 1024 {
		t.Fatalf("factory input=%+v", gotFactory)
	}
...
func TestNewLLMRequestForwardingExecutorRequiresDependencies(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "secrets", factory: validFactory, limit: 1},
		{name: "factory", secrets: validSecrets, limit: 1},
		{name: "limit", secrets: validSecrets, factory: validFactory},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := NewLLMRequestForwardingExecutor(
				test.secrets,
				test.factory,
				test.limit,
			)
...
func mustLLMRequestForwardingExecutor(
	t *testing.T,
	secrets ports.SecretResolver,
	factory ports.ForwardingAdapterFactory,
	limit int64,
...
func validExecutorInput() ForwardingExecutionInput {
	...
	prepared := PreparedRequest{
		...
		Plan: RoutePlan{
			...
			EstimatedUsage:             domain.TokenUsage{InputTokens: 1},
