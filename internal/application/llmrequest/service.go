...
func (s *Service) prepare(
	...
	parsed, err := s.requestParser.Parse(ctx, ParseInput{
	...
	requestedCapabilities, err := s.capabilityDetector.Detect(ctx, CapabilityInput{
		APIFamily:    input.APIFamily,
		EndpointKind: input.EndpointKind,
		PathModel:    input.PathModel,
	...
	plan, err := s.routePlanner.Plan(ctx, RoutePlanInput{
...
func validateInput(input Input) error {
	if !validLocalRequestID(input.LocalRequestID) ||
		...
		input.APIFamily == "" ||
		input.EndpointKind == "" ||
		!validUpstreamPath(input.UpstreamPath) ||
		input.Payload == nil {
