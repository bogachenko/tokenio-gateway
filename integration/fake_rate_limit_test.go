...
func TestFakeServicesRateLimitScenario(t *testing.T) {
	t.Parallel()
	...
	t.Run("billing", func(t *testing.T) {
		...
		assertRateLimit(t, response, err, "rate_limit")
	})
	...
	t.Run("openai compatible", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1/chat/completions", foc.Response{
			Status: http.StatusTooManyRequests,
			Header: rateLimitHeader(),
			Body:   []byte(`{"error":{"message":"rate_limit","type":"rate_limit_exceeded"}}`),
		})
	...
	t.Run("anthropic", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1/messages", fa.Response{
			Status: http.StatusTooManyRequests,
			Header: rateLimitHeader(),
			Body:   []byte(`{"type":"error","error":{"type":"rate_limit_error","message":"rate_limit"}}`),
		})
	...
	t.Run("gemini", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1beta/models/gemini-test:generateContent", fg.Response{
			Status: http.StatusTooManyRequests,
			Header: rateLimitHeader(),
			Body:   []byte(`{"error":{"code":429,"message":"rate_limit","status":"RESOURCE_EXHAUSTED"}}`),
		})
	...
	t.Run("telegram", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/botTEST_TOKEN/sendMessage", ft.Response{
			Status: http.StatusTooManyRequests,
			Header: rateLimitHeader(),
			Body:   []byte(`{"ok":false,"error_code":429,"description":"Too Many Requests: rate_limit"}`),
		})
...
func rateLimitHeader() http.Header {
	return http.Header{
...
func assertRateLimit(t *testing.T, response *http.Response, err error, wantBodyPart string) {
	t.Helper()
	...
	if response.StatusCode != http.StatusTooManyRequests {
		t.Fatalf("status=%d want=%d body=%s", response.StatusCode, http.StatusTooManyRequests, string(body))
	}
