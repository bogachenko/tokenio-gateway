...
func TestFakeServicesInvalidRequestScenario(t *testing.T) {
	t.Parallel()
	...
	t.Run("billing", func(t *testing.T) {
		...
		assertStatusAndBody(t, response, err, http.StatusBadRequest, "invalid_request")
	})
	...
	t.Run("openai compatible", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1/chat/completions", foc.Response{
			Status: http.StatusBadRequest,
			Header: jsonHeader(),
			Body:   []byte(`{"error":{"message":"invalid_request"}}`),
		})
	...
	t.Run("anthropic", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1/messages", fa.Response{
			Status: http.StatusBadRequest,
			Header: jsonHeader(),
			Body:   []byte(`{"type":"error","error":{"type":"invalid_request_error","message":"invalid_request"}}`),
		})
	...
	t.Run("gemini", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1beta/models/gemini-test:generateContent", fg.Response{
			Status: http.StatusBadRequest,
			Header: jsonHeader(),
			Body:   []byte(`{"error":{"code":400,"message":"invalid_request","status":"INVALID_ARGUMENT"}}`),
		})
	...
	t.Run("telegram", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/botTEST_TOKEN/sendMessage", ft.Response{
			Status: http.StatusBadRequest,
			Header: jsonHeader(),
			Body:   []byte(`{"ok":false,"error_code":400,"description":"Bad Request: invalid_request"}`),
		})
...
func assertStatusAndBody(t *testing.T, response *http.Response, err error, wantStatus int, wantBodyPart string) {
	t.Helper()
