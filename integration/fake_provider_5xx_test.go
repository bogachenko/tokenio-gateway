...
func TestFakeServicesProvider5xxScenario(t *testing.T) {
	t.Parallel()
	...
	t.Run("billing", func(t *testing.T) {
		...
		server.SetResponse(fb.Response{
			...
			Header: provider5xxJSONHeader(),
			Body:   []byte(`{"error":"provider_5xx"}`),
		})
		response, err := http.Post(server.URL()+"/v1/charges", "application/json", strings.NewReader(`{}`))
		assertProvider5xx(t, response, err, http.StatusBadGateway, "provider_5xx")
	})
	...
	t.Run("openai compatible", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1/chat/completions", foc.Response{
			...
			Header: provider5xxJSONHeader(),
			Body:   []byte(`{"error":{"message":"provider_5xx","type":"server_error"}}`),
		})
		response, err := http.Post(server.URL()+"/v1/chat/completions", "application/json", strings.NewReader(`{}`))
		assertProvider5xx(t, response, err, http.StatusInternalServerError, "provider_5xx")
	})
	...
	t.Run("anthropic", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1/messages", fa.Response{
			Status: http.StatusInternalServerError,
			Header: provider5xxJSONHeader(),
			Body:   []byte(`{"type":"error","error":{"type":"api_error","message":"provider_5xx"}}`),
		})
	...
	t.Run("gemini", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/v1beta/models/gemini-test:generateContent", fg.Response{
			Status: http.StatusInternalServerError,
			Header: provider5xxJSONHeader(),
			Body:   []byte(`{"error":{"code":500,"message":"provider_5xx","status":"INTERNAL"}}`),
		})
	...
	t.Run("telegram", func(t *testing.T) {
		...
		server.SetResponse(http.MethodPost, "/botTEST_TOKEN/sendMessage", ft.Response{
			Status: http.StatusInternalServerError,
			Header: provider5xxJSONHeader(),
			Body:   []byte(`{"ok":false,"error_code":500,"description":"Internal Server Error: provider_5xx"}`),
		})
...
func provider5xxJSONHeader() http.Header {
	return http.Header{
		"Content-Type": []string{"application/json"},
	}
}

func assertProvider5xx(t *testing.T, response *http.Response, err error, wantStatus int, wantBodyPart string) {
	t.Helper()
