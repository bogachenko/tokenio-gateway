...
| Storage and migration contract | `070`, `090`, migration ADR | `partial`: explicit cmd/migrate ownership, gateway validate-only startup, previous-version upgrade, repeat apply, checksum/ahead rejection, rollback and concurrent migration convergence verified; table/constraint mapping pending |
