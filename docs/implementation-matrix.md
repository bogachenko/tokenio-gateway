...
| Storage and migration contract | `070`, `090`, migration ADR | `partial`: migration lifecycle, exact usage dimensions, immutable ordered charge command, exact float64 markup, SQL-enforced usage CAS, and reserved/billable UTC timestamp nullability verified; remaining table/constraint mapping pending |
